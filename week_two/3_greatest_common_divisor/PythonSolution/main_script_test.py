...
for i in range(tests):
    ...
    os.system("python3 gcdNaive.py < input.txt > gcdNaive.txt")
    # run the main solution 
    os.system("python3 gcdFast.py < input.txt > gcdFast.txt")

    # read the output of the model solution: 
    with open('gcdNaive.txt') as f: model = f.read()
    print("gcdNaive: ", model)
